...
{
	if (!inside_graph(_x, _y))
		return;
	if (i_p_count == MAX_IPOINTS)
		return;
...
void display_input_points(
	int graph_x, int graph_y, int size, ALLEGRO_COLOR c_input, ALLEGRO_COLOR c_loaded
	)
{
	int vertex_x, vertex_y;
	// draw data points with color c_input
	for (int i = 0; i < i_p_count; i++)
	{
		if (!inside_graph(input_points[i][0], input_points[i][1]))
			continue;
		vertex_x = graph_x + input_points[i][0] - size;
		vertex_y = graph_y + input_points[i][1] - size;
		al_draw_filled_rectangle(vertex_x, vertex_y, vertex_x + size, vertex_y + size, c_input);
	}
	// draw dataset over data points with color c_loaded
	// (not all current points are necessarily loaded as ts)
	for (int i = 0; i < get_n_examples(); i++)
	{
		if (!inside_graph(get_example(i)[0], get_example(i)[1]))
			continue;
		vertex_x = graph_x + get_example(i)[0] - size;
		vertex_y = graph_y + get_example(i)[1] - size;
		al_draw_filled_rectangle(vertex_x, vertex_y, vertex_x + size, vertex_y + size, c_loaded);
	}
}
...
void handle_event(ALLEGRO_EVENT event)
{
	...
	case ALLEGRO_EVENT_TIMER:
		...
		if (keys[ALLEGRO_KEY_T] & KEY_DOWN)
			train = true; // TRAIN
		if (keys[ALLEGRO_KEY_L] & KEY_PRESSED)
			load_training_set();
		if (keys[ALLEGRO_KEY_R] & KEY_PRESSED)
			reset();

		if (keys[ALLEGRO_KEY_A] & KEY_PRESSED)
			learn_ts(1); // TRAIN (1 STEP)

		if (keys[ALLEGRO_KEY_D] & (KEY_PRESSED | KEY_DOWN))
			del_input_point();
...
int main()
{
	...
	while (1)
	{
		...
		if (redraw && al_is_event_queue_empty(queue))
		{
			...
			display_input_points(p_off_w, p_off_h, 6, color_accent, color_red);
			// testo
			int line = 0;
			char tmp[100];
			sprintf(tmp, "FIXED VALUES:");
			print_info(font, color_text2, line++, tmp);
			sprintf(tmp, "Learning rate decay = %.3f", LEARN_DECAY);
			print_info(font, color_text, line++, tmp);
			sprintf(tmp, "Radius decay = %.3f", RADIUS_DECAY);
			print_info(font, color_text, line++, tmp);
